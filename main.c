...
int main()
{
	char a[256]={},b[256]={},m[256]={},n[256]={};//一格只傳一個值 ,比int好用 ,最多只有256位數 ,先訂空位,不然會亂數 
	...
	while((int)m[0]-48>0 && (int)n[0]-48>0) 
	{
		...
		if(nLength>mLength) 
		{
			...
			copy(m,temp);
			copy(n,m);
			memset(n, (char)(0), 256);//string.h,把n全歸零,不然copy的話,因為n位數多,無法用到n的尾巴 
			copy(temp,n);
		}
...
void arrange(char m[],int len)//排列,把整個列往前挪
{
	
	for(int i=0;i<len-1;i++ )m[i]=m[i+1];
	if(len<256)m[len-1]=m[len];
	else m[255] = (char)(0);//注意char 0和'0'是不同的,char 0的ascill code =0,扣-48 =-48 
